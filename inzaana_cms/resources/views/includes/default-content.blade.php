<?php //include ("templates/$category_name/$template_name/body.html");?>

<div id="container_background-1" style="width: 100%; height: 1200px;">
       <div id="container_background-1_div-1" style="width: 100%; height: 1200px;">
            <div id="hint" name="textarea" style="position: absolute; width: 70%; height: 300px; border: 2px lightgray dotted; left: 15%; top: 30%;">
                <h1><span style="position: absolute; color: lightgray; left: 33%; top: 35%;">{{ $currentMenu . ': Drop Items Here' }} </span></h1>
            </div>
        </div>
</div>
<div id="footer">
	<?php include ("templates/$category_name/$template_name/footer.html");?>
</div>